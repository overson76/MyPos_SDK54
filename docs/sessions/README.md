...
| 2026-05-15 (2부) | [2026-05-15-2부-OrderFlow-CID자동배달-NSIS-1.0.47to48.md](2026-05-15-2부-OrderFlow-CID자동배달-NSIS-1.0.47to48.md) | **1.0.47-48 주문 흐름 + CID 단골 자동 + NSIS 덮어쓰기 fix**. 사장님 실사용 요청 8개. **1.0.47**: IncomingCallBanner 15초+크게, `findEmptyDeliverySlot` 헬퍼(d1~d5 빈 슬롯), `submitPendingAsDelivery` 액션(PENDING→배달 슬롯 자동 배당+주소/전화/별칭), OrderScreen 주문 버튼 미선택+cart→배달 자동, CID 주문받기→PENDING 자동 박힘, 예약/포장 카드 시간 표기(📅📦), NSIS oneClick true→false + Sleep 5000 + runAfterFinish:true (1.0.11~1.0.46 매번 수동 삭제+재설치 패턴 영원히 졸업). **1.0.48**: CID 단골 15초 무응답 자동 배달 — 매장 PC 만 처리(isElectron 가드)+ts dedupe+단골만(alias 또는 address&&!isNewNumber), cart 비어있어 주방 미등록, 사장님이 메뉴 추가 후 주문 시 confirmOrder→주방 등록. 메모리 추가: 영업시간(일요일 휴무, 10:30~20:00). 폰 OTA push(production, runtime 1.0.2). | ce05673 |
