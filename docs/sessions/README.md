...
| 2026-05-15 (2부) | [2026-05-15-2부-OrderFlow-CID자동배달-NSIS-1.0.47to48.md](2026-05-15-2부-OrderFlow-CID자동배달-NSIS-1.0.47to48.md) | **1.0.47-50 한 세션 네 빌드 — 주문/CID/NSIS/UX 통합**. 사장님 실사용 요청 누적 10개. (1.0.47) IncomingCallBanner 15초+크게, findEmptyDeliverySlot+submitPendingAsDelivery (미선택+cart→배달 1~5 자동 배당), OrderScreen 주문 버튼/App.js CID 주문받기 PENDING 자동 박힘, 예약·포장 카드 시간(📅📦), NSIS oneClick:false+Sleep 5000+runAfterFinish (매번 수동 삭제+재설치 패턴 졸업). (1.0.48) CID 단골 15초 무응답 자동 배달 — isElectron 가드+ts dedupe+단골만, cart 비어있어 주방 미등록. (1.0.49) 배달 카드 별칭 우선 표시(👤 진실보석 ← 📍 사하구 사하로...), KIOSK 자동 풀스크린 제거 → 일반 창 + 자동 최대화(─ □ X 자유). (1.0.50) AddressBookModal 검색 label 전용 → label+alias+phone 3가지 OR 버그 fix, placeholder "주소·별칭·전화번호 검색". 폰 OTA 2회 push (1.0.48 group 1b767bb2, 1.0.50). 영업시간 메모리 추가 (일요일 휴무, 10:30~20:00). | ce05673, c32daf1, c083df5 | **1.0.47**: IncomingCallBanner 15초+크게, `findEmptyDeliverySlot` 헬퍼(d1~d5 빈 슬롯), `submitPendingAsDelivery` 액션(PENDING→배달 슬롯 자동 배당+주소/전화/별칭), OrderScreen 주문 버튼 미선택+cart→배달 자동, CID 주문받기→PENDING 자동 박힘, 예약/포장 카드 시간 표기(📅📦), NSIS oneClick true→false + Sleep 5000 + runAfterFinish:true (1.0.11~1.0.46 매번 수동 삭제+재설치 패턴 영원히 졸업). **1.0.48**: CID 단골 15초 무응답 자동 배달 — 매장 PC 만 처리(isElectron 가드)+ts dedupe+단골만(alias 또는 address&&!isNewNumber), cart 비어있어 주방 미등록, 사장님이 메뉴 추가 후 주문 시 confirmOrder→주방 등록. 메모리 추가: 영업시간(일요일 휴무, 10:30~20:00). 폰 OTA push(production, runtime 1.0.2). | ce05673 |
