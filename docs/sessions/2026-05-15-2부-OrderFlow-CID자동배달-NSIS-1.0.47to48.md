# 2026-05-15 2부 — 주문 흐름 개선 + CID 단골 자동 배달 + NSIS 덮어쓰기 fix (1.0.47-48)

## 한 줄 요약

오전 1부 (LG U+ Webhook 매장 PC 도달 완성) 후 사장님 실사용 요청 6개 + 추가 2개 한 빌드(1.0.47) → 추가 빌드(1.0.48) 로 처리. **미선택+카트+"주문" → 배달 1~5 자동 배당**, **CID "주문받기" → PENDING 주소/전화 자동 박힘**, **CID 단골 15초 무응답 → 자동 배달 슬롯 (메뉴 대기)**, **포장/예약 카드 시간 표기**, **NSIS 덮어쓰기 fix** (다음부터 매번 삭제+재설치 졸업). 폰 OTA push 도 완료 — 폰/직원폰 다음 시작 시 새 흐름 자동 반영.
