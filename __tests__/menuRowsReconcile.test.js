...
describe('reconcileRowsWithDefaults', () => {
  ...
  test('모든 카테고리 메뉴 있음 → recovered=false + 그대로 통과', () => {
    const input = {
      '즐겨찾기': [
    ...
    expect(rows['음료'][0][0]).toBe(30);
  });

  test('카테고리 키는 있지만 격자가 모두 null → default 자동 복원 (2부 사고)', () => {
    // 2026-05-26 2부 사고: listener 가 모든 카테고리 키는 있고 격자만 통째 비워진
    // 데이터를 받음. 1차 fix 의 "키 누락" 만 검사하던 로직은 이걸 못 잡았음.
    const input = {
      '즐겨찾기': emptyGrid(),
      '국수/만백': emptyGrid(),
      '음료': emptyGrid(),
  ...
  test('일부만 빈 격자 → 그 카테고리만 default 복원', () => {
    const input = {
      '즐겨찾기': [
    ...
    const { rows, recovered } = reconcileRowsWithDefaults(input, DEFAULTS, CATS);
    expect(recovered).toBe(true);
    expect(rows['즐겨찾기'][0][0]).toBe(10); // 보존
    expect(rows['국수/만백'][0][0]).toBe(1); // default 복원
    expect(rows['음료'][0][0]).toBe(30); // 보존
  });
